...
- **This actually bit us for real in production** — a handful of registrations ended up with a real Supabase Auth user but no `profiles` row (likely from requests landing mid-restart during local dev testing), permanently stuck: couldn't re-register (email taken) or log in (nothing to read `email_verified` from). The real bug was that `register()` never checked whether the `profiles` insert succeeded. Fixed by rolling back the just-created auth user whenever that insert fails, so the email is free to retry cleanly instead of silently orphaning the account — see `controllers/authController.js`.
