  ...
  return (
    <motion.div ref={cardRef} onMouseMove={handleMouseMove} onMouseLeave={() => setRotation({ x: 0, y: 0 })}
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        {stats.totalSpent === 0 ? (
          <motion.button className="btn btn-mint neopop" onClick={onAddSMS} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} style={{ padding: '12px 24px', fontSize: 14 }}>
            📱 Paste your first SMS →
          </motion.button>
        ) : (
          <div style={{ display: 'flex', gap: 8 }}>
            <motion.button className="btn btn-mint btn-sm neopop" onClick={onAddSMS} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>+ Add SMS</motion.button>
          </div>
        )}
      </div>
    </motion.div>
...
function ReceiptScanner({ onAdd, onClose }) {
    ...
    try {
      ...
      reader.onload = async (e) => {
        try {
          ...
          setResult(r); setCategory(r.category || 'Other')
        } catch (err) { setError(err.response?.data?.message || 'Could not read receipt. Try a clearer photo.') }
        setScanning(false)
      }
      reader.readAsDataURL(image)
    } catch { setError('Something went wrong. Try again.'); setScanning(false) }
  }

  const confirm = async () => {
    if (!result) return
    try {
      await api.post('/transactions', { merchant: result.vendor, amount: result.amount, category, date: result.date, type: 'debit', note: 'Added via receipt scan' })
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 9999, backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}
      onClick={(e) => e.target === e.currentTarget && onClose()}>
      <motion.div initial={{ scale: 0.93, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.93, y: 20 }}
        style={{ background: '#111318', border: '1px solid rgba(0,229,160,0.2)', borderRadius: 24, width: '100%', maxWidth: 460, overflow: 'hidden', boxShadow: '0 40px 100px rgba(0,0,0,0.7)' }}>
        <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border)', background: 'linear-gradient(135deg, rgba(0,229,160,0.08), rgba(124,58,237,0.06))', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
          ...
          {preview && !result && (
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button className="btn btn-mint" onClick={scan} disabled={scanning} style={{ flex: 1, padding: '12px', fontSize: 13 }}>
                {scanning ? <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}><div className="spinner" /> Scanning receipt...</span> : '🔍 Scan with AI'}
              </button>
            </div>
          )}
          {result && (
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 8 }}>
                  {CATS.map(cat => (
                    <button key={cat} onClick={() => setCategory(cat)}
                      style={{ padding: '6px 14px', borderRadius: 20, border: `1px solid ${category === cat ? 'var(--mint)' : 'var(--border)'}`, background: category === cat ? 'var(--mint-dim)' : 'transparent', color: category === cat ? 'var(--mint)' : 'var(--text-3)', fontSize: 12, fontWeight: category === cat ? 700 : 500, cursor: 'pointer', transition: 'all 0.15s' }}>
                      {cat}
                    </button>
                  ))}
                </div>
...
function SMSSection({ onSave, onOpenReceiptScanner }) {
    ...
    try {
      ...
      const otherVendors = savedTxns
        ...
        .reduce((acc, tx) => {
          ...
          if (existing) { existing.ids.push(tx.id) }
          else acc.push({ merchant: tx.merchant, amount: tx.amount, date: tx.date, ids: [tx.id] })
          return acc
        }, [])
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="section" style={{ marginBottom: 24 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <button className="btn btn-outline btn-sm" onClick={onOpenReceiptScanner} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>🧾 Scan Receipt</button>
        </div>
      </div>
      <AnimatePresence mode="wait">
        {tab === 'sms' && (
          <motion.div key="sms" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
            <div>
              ...
              <motion.button className="btn btn-mint" style={{ width: '100%', marginTop: 12, padding: '13px' }} onClick={handleSave} disabled={!parsed.length || saving} whileTap={{ scale: 0.98 }}>
                {saving ? <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}><div className="spinner" />Saving...</span>
        ...
        {tab === 'manual' && (
          <motion.div key="manual" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} style={{ maxWidth: 460, display: 'flex', flexDirection: 'column', gap: 14 }}>
...
function TxnsSection({ transactions, onDelete }) {
  ...
  const filtered = transactions.filter(tx =>
    tx.merchant.toLowerCase().includes(search.toLowerCase()) && (filter === 'All' || tx.category === filter)
  )
  const displayed = showAll ? filtered : filtered.slice(0, 8)
  const merchantCounts = {}
  transactions.forEach(tx => { const key = tx.merchant.toLowerCase().split(' ')[0]; merchantCounts[key] = (merchantCounts[key] || 0) + 1 })
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <AnimatePresence>
              {displayed.map((tx, i) => {
                ...
                return (
                  <motion.div key={tx.id} className="tx-row" layout initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20, height: 0 }} transition={{ delay: i * 0.025 }} style={{ justifyContent: 'space-between' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <motion.div className="cat-dot" whileHover={{ scale: 1.15, rotate: 10 }} style={{ background: conf.bg, color: conf.color, width: 40, height: 40, borderRadius: 12 }}>{conf.emoji}</motion.div>
                      <div>
...
function weightedForecast(transactions, today, daysInMonth) {
  const dailyMap = {}
  transactions.forEach(tx => { const day = new Date(tx.date).getDate(); dailyMap[day] = (dailyMap[day] || 0) + Number(tx.amount) })
  let weightedSum = 0, totalWeight = 0
  for (let day = 1; day <= today; day++) {
    if (dailyMap[day]) { weightedSum += dailyMap[day] * day; totalWeight += day }
  }
  const weightedDailyAvg = totalWeight > 0 ? weightedSum / totalWeight : 0
  const currentSpend = Object.values(dailyMap).reduce((s, v) => s + v, 0)
  const projected = Math.round(currentSpend + weightedDailyAvg * (daysInMonth - today))
  let cumulative = 0
  const chartData = []
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    if (day <= today) {
      ...
      if (day === today + 1) chartData[chartData.length - 1].forecast = cumulative
      chartData.push({ day: `${day}`, actual: null, forecast: Math.round(currentSpend + weightedDailyAvg * (day - today)) })
    }
  }
...
function ForecastSection({ transactions, stats }) {
  ...
  const thisMonth = transactions.filter(tx => { const d = new Date(tx.date); return d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear() })

  if (thisMonth.length < 3) return (
    <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="section" style={{ marginBottom: 24 }}>
      <div className="section-title" style={{ marginBottom: 4 }}>🔮 Spend Forecast</div>
      <div className="section-sub" style={{ marginBottom: 20 }}>Weighted moving average prediction</div>
  ...
  const { projected: forecastEnd, currentSpend, weightedDailyAvg, chartData } = weightedForecast(thisMonth, today, daysInMonth)
  const ratio = forecastEnd / (currentSpend * (daysInMonth / Math.max(today, 1)))
  let verdict, verdictColor, verdictBg, verdictBorder
  if (ratio <= 1.05) { verdict = "🟢 On track — you're spending consistently"; verdictColor = '#00e5a0'; verdictBg = 'rgba(0,229,160,0.06)'; verdictBorder = 'rgba(0,229,160,0.2)' }
  else if (ratio <= 1.25) { verdict = '🟡 Careful — spending is picking up pace'; verdictColor = '#f59e0b'; verdictBg = 'rgba(245,158,11,0.06)'; verdictBorder = 'rgba(245,158,11,0.2)' }
  else { verdict = '🔴 High spend detected — consider cutting back'; verdictColor = '#ef4444'; verdictBg = 'rgba(239,68,68,0.06)'; verdictBorder = 'rgba(239,68,68,0.2)' }
  const projectedRemaining = Math.round(weightedDailyAvg * (daysInMonth - today))
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="section" style={{ marginBottom: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{ width: 28, height: 3, background: '#00e5a0', borderRadius: 2 }} /><span style={{ fontSize: 11, color: 'var(--text-3)' }}>Actual</span>
      ...
      <div style={{ height: 260, marginBottom: 16 }}>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
            ...
            <XAxis dataKey="day" tick={{ fill: '#475569', fontSize: 10, fontFamily: 'JetBrains Mono' }} axisLine={false} tickLine={false} interval={Math.floor(daysInMonth / 6)} />
            <YAxis tick={{ fill: '#475569', fontSize: 10 }} axisLine={false} tickLine={false} tickFormatter={v => `₹${v >= 1000 ? (v/1000).toFixed(1)+'k' : v}`} />
            <Tooltip contentStyle={{ background: '#1a1d24', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, fontSize: 12, fontFamily: 'JetBrains Mono' }} labelStyle={{ color: '#64748b', marginBottom: 4 }} formatter={(value, name) => [`₹${Number(value).toLocaleString('en-IN')}`, name === 'actual' ? 'Actual' : 'Forecast']} labelFormatter={label => `Day ${label}`} />
            <Line type="monotone" dataKey="actual" stroke="#00e5a0" strokeWidth={2.5} dot={false} activeDot={{ r: 4, fill: '#00e5a0' }} connectNulls={false} />
            <Line type="monotone" dataKey="forecast" stroke="#7c3aed" strokeWidth={2} strokeDasharray="6 4" dot={false} activeDot={{ r: 4, fill: '#7c3aed' }} connectNulls={false} />
          </LineChart>
        </ResponsiveContainer>
      </div>
      <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
        style={{ padding: '14px 18px', background: verdictBg, border: `1px solid ${verdictBorder}`, borderRadius: 12, fontSize: 13, fontWeight: 600, color: verdictColor, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <span>{verdict}</span>
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 14 }}>{forecastEnd > currentSpend ? `+${formatCurrency(forecastEnd - currentSpend)} more projected` : 'Looking good!'}</span>
      </motion.div>
    </motion.div>
  )
}
    ...
    <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 24 }}>
      <div className="section">
        ...
        <div style={{ height: 200 }}>
          <ResponsiveContainer><PieChart><Pie data={catData} cx="50%" cy="50%" innerRadius={55} outerRadius={88} paddingAngle={3} dataKey="value">{catData.map((_, i) => <Cell key={i} fill={CHART_COLORS[i % CHART_COLORS.length]} />)}</Pie><Tooltip content={<ChartTip />} /></PieChart></ResponsiveContainer>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 8 }}>
          {catData.sort((a, b) => b.value - a.value).slice(0, 4).map((d, i) => (
            <div key={d.name} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><div style={{ width: 8, height: 8, borderRadius: '50%', background: CHART_COLORS[i % CHART_COLORS.length] }} />{CATEGORIES[d.name]?.emoji} {d.name}</div>
        ...
        <div style={{ height: 280 }}>
          <ResponsiveContainer>
            <BarChart data={weeklyData} barSize={42}>
              ...
              <Bar dataKey="amount" radius={[6, 6, 0, 0]}>{weeklyData.map((_, i) => <Cell key={i} fill={i === 3 ? '#00e5a0' : 'rgba(0,229,160,0.3)'} />)}</Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
...
function BudgetSection({ stats }) {
    ...
    const fetchBudgets = async () => {
      try {
        ...
        if (res.data.budgets.length > 0) {
          const map = {}; res.data.budgets.forEach(b => { map[b.category] = b.monthly_limit }); setBudgets(map); return
        }
        const lastMonth = now.getMonth() === 0 ? 12 : now.getMonth()
        const lastYear = now.getMonth() === 0 ? now.getFullYear() - 1 : now.getFullYear()
        const prev = await api.get(`/budgets?month=${lastMonth}&year=${lastYear}`)
        if (prev.data.budgets.length === 0) return
        await Promise.all(prev.data.budgets.map(b => api.post('/budgets', { category: b.category, monthlyLimit: b.monthly_limit, month: now.getMonth(), year: now.getFullYear() })))
        const fresh = await api.get(`/budgets?month=${now.getMonth()}&year=${now.getFullYear()}`)
        const map = {}; fresh.data.budgets.forEach(b => { map[b.category] = b.monthly_limit }); setBudgets(map)
  ...
  const save = async () => {
    try {
      await Promise.all(Object.entries(temp).filter(([, v]) => v).map(([category, limit]) => api.post('/budgets', { category, monthlyLimit: Number(limit), month: now.getMonth(), year: now.getFullYear() })))
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="section" style={{ marginBottom: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
        <div><div className="section-title">🎯 Budget Goals</div><div className="section-sub">Monthly limits per category</div></div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 12 }}>
        {cats.map((cat, i) => {
          ...
          return (
            <motion.div key={cat} className="card" initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.04 }} whileHover={{ y: -2 }} style={{ padding: 16 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><span style={{ fontSize: 18 }}>{conf.emoji}</span><span style={{ fontWeight: 600, fontSize: 13 }}>{cat}</span></div>
              ...
              {editing ? <input className="input" type="number" placeholder="Monthly limit ₹" style={{ fontSize: 13, padding: '8px 12px' }} value={temp[cat] || ''} onChange={e => setTemp({ ...temp, [cat]: e.target.value })} /> : (
                <>
                  <div className="progress-track"><motion.div className={`progress-fill ${pClass(pct)}`} initial={{ width: 0 }} whileInView={{ width: `${pct}%` }} viewport={{ once: true }} transition={{ duration: 1.2, ease: [0.4, 0, 0.2, 1] }} /></div>
                  <div style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 5 }}>{limit > 0 ? `${formatCurrency(spent)} of ${formatCurrency(limit)}` : 'No limit — tap Set Limits'}</div>
                  {pct >= 80 && limit > 0 && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ marginTop: 6, fontSize: 11, color: pct >= 100 ? '#ef4444' : '#f59e0b', fontWeight: 600 }}>{pct >= 100 ? '🚨 Over budget!' : '⚠️ Almost there!'}</motion.div>}
                </>
              )}
            </motion.div>
          )
...
function AIBubble({ stats, transactions }) {
  ...
  const generate = async () => {
    ...
    try {
      const res = await api.post('/ai/insight', { totalSpent: stats.totalSpent, byCategory: stats.byCategory, transactionCount: stats.count, topCategory: stats.topCategory })
      const text = res.data.insight; setInsight(text)
  ...
  return (
    <>
      <AnimatePresence>
        {open && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setOpen(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 9997, backdropFilter: 'blur(4px)' }} />}
      </AnimatePresence>
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0, y: 20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.95 }} transition={{ type: 'spring', stiffness: 300, damping: 25 }}
            ...
            <div style={{ background: 'linear-gradient(135deg, rgba(0,229,160,0.12), rgba(124,58,237,0.08))', padding: '16px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div><div style={{ fontFamily: 'Plus Jakarta Sans', fontSize: 15, fontWeight: 700 }}>SpendWise AI</div><div style={{ fontSize: 11, color: '#00e5a0', fontWeight: 600 }}>● online · Powered by Groq</div></div>
                </div>
            ...
            <div style={{ padding: '16px', minHeight: 150, maxHeight: 380, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 10 }}>
              {!hasGenerated && !loading && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ textAlign: 'center', padding: '24px 16px' }}>
                  <div style={{ fontSize: 40, marginBottom: 12 }}>🤖</div>
                  <div style={{ fontSize: 13, lineHeight: 1.7, color: 'var(--text-2)' }}>Hey! I'm your AI finance buddy.<br />I'll <strong style={{ color: '#ef4444' }}>roast your spending</strong> and give you <strong style={{ color: '#00e5a0' }}>actual tips</strong> — not boring bank advice.</div>
                </motion.div>
              )}
              {loading && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {[1, 2, 3].map(i => <motion.div key={i} animate={{ opacity: [0.4, 0.8, 0.4] }} transition={{ duration: 1.5, repeat: Infinity, delay: i * 0.2 }} style={{ height: 64, borderRadius: 14, background: 'var(--surface-2)', border: '1px solid var(--border)' }} />)}
              ...
              {cards.map((card, i) => i < visibleCards && (
                <motion.div key={i} initial={{ opacity: 0, y: 12, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.35 }}
                  ...
                  <div style={{ width: 32, height: 32, borderRadius: 10, flexShrink: 0, background: `${card.color}22`, border: `1px solid ${card.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 }}>{card.emoji}</div>
                  <div style={{ fontSize: 13, lineHeight: 1.65, color: 'var(--text-2)', paddingTop: 2 }}>{card.emoji === '⚡' ? <strong style={{ color: card.color }}>{card.text}</strong> : card.text}</div>
                </motion.div>
              ))}
            </div>
            <div style={{ padding: '10px 14px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
              <motion.button className="btn btn-mint" onClick={generate} disabled={loading} style={{ width: '100%', padding: '12px', fontSize: 13 }} whileTap={{ scale: 0.97 }}>
                {loading ? <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}><div className="spinner" />Thinking...</span> : hasGenerated ? '🔄 Ask again' : '⚡ Get my roast'}
              </motion.button>
            </div>
            <div style={{ padding: '0 16px 14px', fontSize: 11, color: 'var(--text-3)', textAlign: 'center' }}>🔒 Only spending totals sent · SMS stays on your device</div>
          </motion.div>
        )}
      </AnimatePresence>
      <motion.button onClick={() => setOpen(!open)} animate={{ scale: open ? 0.9 : 1 }} whileHover={{ scale: 1.08 }} whileTap={{ scale: 0.92 }}
        style={{ position: 'fixed', bottom: 24, right: 24, zIndex: 9999, width: 58, height: 58, borderRadius: '50%', border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg, #00e5a0, #00c5ff)', boxShadow: '0 8px 32px rgba(0,229,160,0.4)', fontSize: 26, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <motion.span animate={{ rotate: open ? 90 : 0 }} transition={{ duration: 0.2 }}>{open ? '✕' : '🤖'}</motion.span>
      </motion.button>
      {!open && <motion.div animate={{ scale: [1, 1.6, 1], opacity: [0.4, 0, 0.4] }} transition={{ duration: 2.5, repeat: Infinity }} style={{ position: 'fixed', bottom: 24, right: 24, zIndex: 9998, width: 58, height: 58, borderRadius: '50%', border: '2px solid rgba(0,229,160,0.4)', pointerEvents: 'none' }} />}
    </>
  )
}
...
function MonthSwitcher({ selectedMonth, selectedYear, onChange }) {
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    options.push({ month: d.getMonth() + 1, year: d.getFullYear(), label: MONTH_NAMES[d.getMonth()] })
  }
  const currentM = new Date().getMonth() + 1, currentY = new Date().getFullYear()
  return (
    <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} style={{ display: 'flex', gap: 8, marginBottom: 20, overflowX: 'auto', scrollbarWidth: 'none' }}>
      {options.map(({ month, year, label }) => {
        ...
        return (
          <motion.button key={`${month}-${year}`} onClick={() => onChange(month, year)} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
            style={{ padding: '7px 18px', borderRadius: 20, border: `1px solid ${isActive ? '#00e5a0' : 'rgba(255,255,255,0.08)'}`, background: isActive ? 'rgba(0,229,160,0.12)' : 'transparent', color: isActive ? '#00e5a0' : '#475569', fontSize: 13, fontWeight: isActive ? 700 : 400, fontFamily: 'var(--font-body)', cursor: 'pointer', whiteSpace: 'nowrap', transition: 'all 0.2s', display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
            {label} {year !== currentY && <span style={{ fontSize: 10, opacity: 0.7 }}>{year}</span>}
            {isCurrent && <span style={{ fontSize: 9, background: '#00e5a0', color: '#000', borderRadius: 4, padding: '1px 5px', fontWeight: 800, letterSpacing: 0.5 }}>NOW</span>}
          </motion.button>
        )
      })}
    </motion.div>
...
export default function Dashboard() {
  ...
  if (loading) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', background: 'var(--bg)', flexDirection: 'column', gap: 16 }}>
      <motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: 'linear' }} style={{ width: 40, height: 40, border: '3px solid rgba(0,229,160,0.15)', borderTop: '3px solid #00e5a0', borderRadius: '50%' }} />
      <motion.div animate={{ opacity: [0.5, 1, 0.5] }} transition={{ duration: 1.5, repeat: Infinity }} style={{ color: 'var(--text-3)', fontSize: 14 }}>Loading your finances...</motion.div>
    </div>
  )
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh', position: 'relative' }}>
      ...
      <main style={{ maxWidth: 1100, margin: '0 auto', padding: '80px 20px 100px', position: 'relative', zIndex: 1 }}>
        <MonthSwitcher selectedMonth={selectedMonth} selectedYear={selectedYear} onChange={(m, y) => { setSelectedMonth(m); setSelectedYear(y) }} />
        <div id="hero">
        ...
        <div id="sms-section">
          <SMSSection
            onSave={(otherVendors) => {
              fetchData()
              if (otherVendors?.length > 0) { setUnknownVendors(otherVendors); setShowVendorPopup(true) }
            }}
      ...
      <AnimatePresence>
        {showReceiptScanner && <ReceiptScanner onAdd={fetchData} onClose={() => setShowReceiptScanner(false)} />}
      </AnimatePresence>
      <AnimatePresence>
        {showVendorPopup && unknownVendors.length > 0 && (
          <UnknownVendorPopup vendors={unknownVendors} onFetchData={fetchData} onComplete={() => { setShowVendorPopup(false); setUnknownVendors([]) }} />
        )}
      </AnimatePresence>
