    ...
    <div className="auth-wrap" style={{ background: 'var(--bg)', position: 'relative' }}>
      ...
      <motion.div initial={{ opacity: 0, y: 24, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.5 }} style={{ width: '100%', maxWidth: 420 }}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          <div style={{ color: 'var(--text-3)', fontSize: 14 }}>Create your account. It's free forever.</div>
        </div>
        <div className="auth-card">
          <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            <div>
              <label className="input-label">Your name</label>
              <input className="input" type="text" placeholder="Shubh Kaushal" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} />
            </div>
            <div>
          ...
          <div style={{ textAlign: 'center', marginTop: 22, fontSize: 14, color: 'var(--text-3)' }}>
            Already in?{' '}
            <Link to="/login" style={{ color: 'var(--mint)', textDecoration: 'none', fontWeight: 600 }}>Sign in</Link>
          </div>
