      ...
      <motion.div initial={{ opacity: 0, y: 24, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.5 }} style={{ width: '100%', maxWidth: 420 }}>
        ...
        <div className="auth-card">
          <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            ...
            <div><label className="input-label">Password</label><input className="input" type="password" placeholder="••••••••" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} /></div>
          ...
          <div style={{ textAlign: 'center', marginTop: 22, fontSize: 14, color: 'var(--text-3)' }}>
            No account? <Link to="/register" style={{ color: 'var(--mint)', textDecoration: 'none', fontWeight: 600 }}>Create one free</Link>
          </div>
        </div>
      </motion.div>
    </div>
